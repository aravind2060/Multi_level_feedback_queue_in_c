...
void completeProcessInQueue(struct Queue *sourceQueue, struct Queue *targetQueue,struct Queue *storeCompletedProcess)
{
    ...
    if (targetQueue != NULL)
    {
        ...
        {
			if(targetQueue==globalqueue0 && targetQueue->head!=NULL){
		   completeProcessInQueue(targetQueue,sourceQueue,storeCompletedProcess);
		   return;
	        } 
...
void performMultiLevelFeedback(struct Queue *queue0, struct Queue *queue1, struct Queue *fcfs,struct Queue *storeCompletedProcess)
{
        ...
        while (queue0->head!=NULL || queue1->head!=NULL)
		{
		    completeProcessInQueue(queue0, queue1,storeCompletedProcess);
            completeProcessInQueue(queue1, queue0,storeCompletedProcess);
		}
...
void clearMemory(struct Queue *queue0, struct Queue *queue1, struct Queue *fcfs,struct Queue *storeCompletedProcess){

  free(queue0);
  free(queue1);
  free(fcfs);
  free(storeCompletedProcess);
} 
int main()
{
    ...
    clearMemory(queue0,queue1,fcfs,storeCompletedProcess);
    return 0;
}
